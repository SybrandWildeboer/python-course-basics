...
Two tasks. The first one you can definitely finish. The second one you might not — that is
deliberate, and where you get stuck is what we open session 2 with.

Budget: 2–4 hours. **Saturday:** send one line — `Homework done` or `Homework not done`.
...
f-string round to 1 decimal" — looking things up is a real skill and you should start now.
...
Both files live in `sessions/session-01/homework/`. From session 5 onward you will commit
them to git; for now, just keep them in the folder.
